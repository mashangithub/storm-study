...
/**
 * 流分组之ShuffleGrouping：
 *  上游组件中emit的数据，会随机的进入下游的各个组件的不同的线程对应的task中
 *
 *  假设上游有1个spout对应executor
 *      下游有3个bolt对应的executor
 *
 */
public class ShuffleGroupingRealTimeOrderSumTopology {
    ...
    static class OrderBolt extends BaseRichBolt {
        ...
        public void execute(Tuple input) {
            ...
            System.out.println("=====>当前线程id：" + id + "，获得上游的数据为：" + num + "--->" + MyStormUtil.dateFormat() + "-->OLD 李商城实时订单总交易金额为：" + sum + "￥");
            MyStormUtil.sleep(1000);
        }
    ...
    public static void main(String[] args) throws  Exception {
        ...
        tb.setBolt("id_bolt", new OrderBolt(), 3).shuffleGrouping("id_spout");//流分组

        //第二步：使用TopologyBuilder构建Topology
        StormTopology stormTopology = tb.createTopology();
        //第三步：提交作业，可以使用集群模式、本地模式
        //Topology名称
        String topologyName = ShuffleGroupingRealTimeOrderSumTopology.class.getSimpleName();
        //storm配置信息
