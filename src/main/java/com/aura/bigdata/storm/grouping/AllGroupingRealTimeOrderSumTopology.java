...
/**
 * 流分组之AllGrouping：
 *  上游组件发送出去的数据，下游每一个组件都会接收一边，为了ha，设置allGrouping()
 *
 *  假设上游有1个spout对应executor
 *      下游有3个bolt对应的executor
 *
 */
public class AllGroupingRealTimeOrderSumTopology {
    ...
    public static void main(String[] args) throws  Exception {
        ...
        tb.setBolt("id_bolt", new OrderBolt(), 3).allGrouping("id_spout");//流分组

        //第二步：使用TopologyBuilder构建Topology
        StormTopology stormTopology = tb.createTopology();
        //第三步：提交作业，可以使用集群模式、本地模式
        //Topology名称
        String topologyName = AllGroupingRealTimeOrderSumTopology.class.getSimpleName();
        //storm配置信息
