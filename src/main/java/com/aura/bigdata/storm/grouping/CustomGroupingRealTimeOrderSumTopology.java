...
/**
 * 流分组之CustomGrouping：
 *  模拟其中的一个shuffleGrouping/GlobalGrouping
 *
 */
public class CustomGroupingRealTimeOrderSumTopology {
    ...
    public static void main(String[] args) throws  Exception {
        ...
        tb.setBolt("id_bolt", new OrderBolt(), 3).customGrouping("id_spout", new MyCustomGrouping());//流分组

        //第二步：使用TopologyBuilder构建Topology
        StormTopology stormTopology = tb.createTopology();
        //第三步：提交作业，可以使用集群模式、本地模式
        //Topology名称
        String topologyName = CustomGroupingRealTimeOrderSumTopology.class.getSimpleName();
        //storm配置信息
...
/**
 * 使用Custom去模拟GlobalGrouping，所有上游的数据，只进入下游中taskid最小的一个task中
 */
class MyCustomGrouping implements CustomStreamGrouping {
    ...
    public void prepare(WorkerTopologyContext context, GlobalStreamId stream,
        ...
        this.tasks = new ArrayList<>(targetTasks.size());
